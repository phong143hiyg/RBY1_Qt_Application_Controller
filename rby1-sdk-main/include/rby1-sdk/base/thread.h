 ...
 private:
  ...
  void SetThreadName() {
    if (!name_.empty()) {
      ...
      // MinGW cannot compile the MSVC structured exception handler below.
      // SetThreadDescription is optional on older Windows versions.
      const auto kernel = GetModuleHandleW(L"kernel32.dll");
      if (kernel) {
        using SetThreadDescriptionFn = HRESULT(WINAPI*)(HANDLE, PCWSTR);
        const auto set_description = reinterpret_cast<SetThreadDescriptionFn>(
            GetProcAddress(kernel, "SetThreadDescription"));
        const int length = MultiByteToWideChar(CP_UTF8, 0, name_.c_str(), -1, nullptr, 0);
        if (set_description && length > 0) {
          std::wstring wide_name(length, L'\0');
          MultiByteToWideChar(CP_UTF8, 0, name_.c_str(), -1, wide_name.data(), length);
          set_description(GetCurrentThread(), wide_name.c_str());
        }
      }
#elif defined(_WIN32)
